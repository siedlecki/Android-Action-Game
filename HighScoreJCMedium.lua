...
-- -----------------------------------------------------------------------------------
 ...
 local sciezka = system.pathForFile("ScoreJCMedium.json", system.DocumentsDirectory)
...
local function wczytajWyniki()
    ...
    if file then --jezęli plik istnieje to wyczytujemy jego dane
...
local function gotoMenu()
    ...
    composer.removeScene("HighScoreJCMedium")  --usunięcie sceny by zwolnić pamięć i przejście do następnej
    composer.setVariable("fwynikC2", 0) --wyzerowanie zmiennej wyniku by wpisywać tej samej wartości przy wielokrotnym wejściu do HighScore
...
local function restart()
    ...
    composer.removeScene("HighScoreJCMedium")
    composer.setVariable("fwynikC2", 0)
...
function scene:create( event )
    ...
    wczytajWyniki()
    table.insert(tabelaWynikow, composer.getVariable("fwynikC2"))
    ...
    local HighScoreHeader = display.newText(sceneGroup, "High Scores Jelly Catcher[Medium]", display.contentCenterX, 10, nil, 25)
    ...
    local mButto = widget.newButton{
        ...
        width=154, height=40,
        onRelease = gotoMenu   --nasłuchiwanie zdarzenias
    }
