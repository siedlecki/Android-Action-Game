...
local backgroundMusic = audio.loadStream( "unlimited.mp3" )
...
local function move(event)
	...
	local function gameOver()
		...
		composer.setVariable("fwynik", score)  --przesłanie wyniku
		composer.removeScene("unlimited")  --usunięcie sceny by zwolnić pamięć
        composer.gotoScene( "HighScoreUnlimited", "fade", 500 )  --przejście do sceny wyników
	end
...
local function badJellyTouched(event)
	if ( event.phase == "began" ) then
		if(gamestate=="paused")then
		else
...
local function resumeTouched (event)
	...
	audio.play(click2 ,{channel=2})  --dzwięk kliknięcia
...
local function newgameTouched (event)
	...
	composer.removeScene("unlimited")
...
local function addNewObject()
	...
	elseif(a>25 and a<=45)then --losowanie zlej żelki
		...
		local badjelly = display.newImage(g, startX, -300) --tworzenie obiektu bad jelly
			...
			badjelly:addEventListener( "touch", badJellyTouched )
	...
	elseif(a>48 and a<=51)then --tworzenie obiektu powerJelly
			...
			local b=math.random(1,5) --losowanie koloru bad jelly
			if (b==1) then
				g="jelly-blue.png"
			elseif(b==2)then
				g="jelly-green.png"
