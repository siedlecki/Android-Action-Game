...
--używam bibloteki Json do zapisywania wyników do plików
 ...
 local sciezka = system.pathForFile("UnlimitedScore.json", system.DocumentsDirectory)
...
local function zapiszWyniki()
    ...
    file:write(temp) --zapisujemy wartość do pliku
...
local function gotoMenu()
    audio.play(click ,{channel=2}) --wywołanie dźwięku po kliknięciu na kanale drugi
    composer.removeScene("HighScoreUnlimited") --usunięcie sceny by zwolnić pamięć i przejście do następnej
    composer.setVariable("fwynik", 0) --wyzerowanie zmiennej wyniku by wpisywać tej samej wartości przy wielokrotnym wejściu do HighScore
...
local function restart()
	...
	composer.removeScene("HighScoreUnlimited")
...
function scene:create( event )
    ...
    wczytajWyniki()
    table.insert(tabelaWynikow, composer.getVariable("fwynik"))
    ...
    local HighScoreHeader = display.newText(sceneGroup, "High Scores", display.contentCenterX, 10, nil, 30)
    ...
    mButto.x = display.contentCenterX+80  --nasłuchiwanie zdarzenia
