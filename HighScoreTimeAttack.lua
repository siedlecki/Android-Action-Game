...
--używam bibloteki Json do zapisywania wyników do plików
 ...
 local sciezka = system.pathForFile("ScoreTimeAttack.json", system.DocumentsDirectory)
...
local function gotoMenu()
    ...
    composer.removeScene("HighScoreTimeAttack") --usunięcie sceny by zwolnić pamięć i przejście do następnej
    composer.setVariable("fwynik2", 0) --wyzerowanie zmiennej wyniku by wpisywać tej samej wartości przy wielokrotnym wejściu do HighScore
...
local function restart()
    ...
    composer.removeScene("HighScoreTimeAttack")
...
function scene:create( event )
    ...
    wczytajWyniki()
    table.insert(tabelaWynikow, composer.getVariable("fwynik2"))
    ...
    local HighScoreHeader = display.newText(sceneGroup, "High Scores Time Attack", display.contentCenterX, 10, nil, 25)
