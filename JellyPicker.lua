...
function scene:create( event )
    ...
    local function gameOver()
        ...
        composer.setVariable("fwynik3", score) --przesłanie wyniku
        composer.removeScene("JellyPicker") --usunięcie sceny by zwolnić pamięć
        composer.gotoScene( "HighScoreJellyPicker", "fade", 500 )  --przejście do sceny wyników
    end
    ...
    local function newgameTouched (event)
        ...
        composer.removeScene("JellyPicker")
        ...
        local b=math.random(1,5) --losujemy kolor jar
            ...
            elseif(c==3)then
                if(PositionCCheck==false)then
                    ...
                    if(time%60==0)then --zmieniamy prędkość pojawiania się jar zależnie od czasu
                    ...
                    timerzx=timer.performWithDelay(6000,   function() if(jar.removeSelf~=nil)then
                    if(gamestate~="paused")then
...
local function addNewObject2()
    ...
    if (jellycheck<4)then
        ...
        local d=math.random(1,10) --losowanie między kamieniem a żelką
            if(d==10)then
                if(c==1)then
