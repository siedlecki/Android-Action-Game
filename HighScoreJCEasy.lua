...
--używam bibloteki Json do zapisywania wyników do plików
 ...
 local sciezka = system.pathForFile("ScoreJCEasy.json", system.DocumentsDirectory)
...
local function gotoMenu()
    ...
    composer.removeScene("HighScoreJCEasy")  --usunięcie sceny by zwolnić pamięć i przejście do następnej
    composer.setVariable("fwynikC1", 0) --wyzerowanie zmiennej wyniku by wpisywać tej samej wartości przy wielokrotnym wejściu do HighScore
...
local function restart()
    ...
    composer.removeScene("HighScoreJCEasy")
...
function scene:create( event )
    ...
    wczytajWyniki()
    table.insert(tabelaWynikow, composer.getVariable("fwynikC1")) --wynik z ostatniej gry zostaje dodany do tabeli
    ...
    local HighScoreHeader = display.newText(sceneGroup, "High Scores Jelly Catcher", display.contentCenterX, 10, nil, 25)
